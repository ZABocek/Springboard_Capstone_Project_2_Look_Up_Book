...
const SearchBooks = () => {
  ...
  const getPrefix = (lastName) => {
    const cleaned = (lastName || '').trim().toUpperCase().replaceAll(/[^A-Z]/g, '');
    if (!cleaned) return '#';
  ...
  const narrowedBooks = useMemo(() => {
    ...
    return prefixFiltered.filter((book) => book.searchBlob.includes(trimmedQuery));
  }, [booksForLetter, selectedPrefix, query]);
  ...
  useEffect(() => {
    setAuthorPage(1);
    setBooksPage(1);
  }, [selectedLetter, selectedPrefix, query]);
  ...
  return (
    <div className="add-db-book-container">
      ...
      <p className="subtitle">
        Browse prize-winning books already in the database. Career-award-only entries are hidden here.
      </p>

      <div className="lookup-controls">
        <label htmlFor="search-books-query" className="lookup-label">
          Quick search within selected letter and sublist
        </label>
        <input
          id="search-books-query"
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Type author last name, first name, title, genre, year, or prize"
          className="lookup-search"
        />
      </div>

      <div className="letter-grid" aria-label="Author last-name initials">
      ...
      <div className="sublist-grid" aria-label="Author last-name prefixes">
            ...
            {visibleBooks.length > 0 ? (
              visibleBooks.map((book) => (
                <div key={`${book.book_id}-${book.prize_name}-${book.prize_year}`} className="book-item">
                  ...
                  <div>{(book.prize_year || book.display_year) ? `Award Year: ${book.prize_year || book.display_year}` : 'Award Year: Unknown'}</div>
                </div>
              ))
            ) : (
